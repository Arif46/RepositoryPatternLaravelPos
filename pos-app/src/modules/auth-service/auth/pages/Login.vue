   ...
   <b-container fluid class="px-0 mc-login-bg">
     <b-row align-v="center" no-gutters>
        <b-col md="6" class="d-none d-md-block">
            ...
            <h4 class="text-center mt-3 mb-2">{{ $t('globalTrans.project_name') }}</h4>
            <h5 class="text-center mb-3">({{ $t('globalTrans.moc') }})</h5>
        </b-col>
            ...
            <div class="login-wrapper">
              <b-overlay :show="loading">
                <b-card>
                  ...
                  <h5 class="form-logo-text text-center mt-2 mb-3">{{ $t('globalTrans.moc') }}</h5>
                  ...
                  <ValidationObserver ref="form" v-slot="{ handleSubmit, reset }">
                    <b-form @submit.prevent="handleSubmit(login)" @reset.prevent="reset" autocomplete="off">
                      <ValidationProvider name="Username/Email" vid="username" rules="required">
                        <b-form-group
                            id="user"
    ...
    async login () {
      ...
      if (result.success) {
        ...
        if (result.user_type === 1) { // 1 mean admin user
          ...
          this.$router.push('/external-user/profile')
        }
      } else {
        this.errorAlert = true
        this.errorMsg = this.$t('globalTrans.login_failed')
